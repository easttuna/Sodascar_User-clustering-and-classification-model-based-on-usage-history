...
`본 프로젝트에서는 SOCAR 유저들의 이용이력에 기반하여 설명가능하며, 비즈니스적으로 의미 있는 유저 유형을 도출하고자 하였습니다. 또한 유저의 일부 이용이력으로 유저 유형을 분류는 모델을 만듦으로써 신속하게 유저유형을 파악할수 있도록 하였습니다. `
...
- ‘**user 유형**’을 정답라벨로 부여하고, user의 특성으로 이를 분류하는 Multiclass Classification 모델 생성 및 검증
- user의 특성은 이용정보에 존재하는 각 user 별 **최초 5회의 이용이력**만으로 생성하여 분류결과의 활용 가능성 확인  
...
>- 또한 **일부의 이용이력**만으로 전체 이용이력으로 부여된 **정답라벨(이용자 유형)를 예측**하는 모델을 생성하여, **유저유형을 빠르게 파악하고 대응**하는데에 도움이 되고자 하였습니다. 
>- 특히 업무 등 필요에 의해 쏘카를 빌리기보다 **여가 자체**를 위해 이용하는 유저군집에게는, **목적지 추천**과 **게임화 서비스** 등을 통해 **이용주기**를 더욱 **단축**시킬 여지가 있습니다.
...
>- 또한, 유저 유형 군집화 및 예측 프레임워크에 포함된 아이디어가 보다 상세한 이용이력을 바탕으로한 **개인화 추천**에도 도움이 될 수 있기를 바랍니다.
